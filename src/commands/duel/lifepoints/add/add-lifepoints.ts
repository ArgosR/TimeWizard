...
export class AddLifepoints implements ILeafCommand {
  ...
  buildSchema(): SlashCommandSubcommandBuilder {
    ...
    return new SlashCommandSubcommandBuilder()
      ...
      .addNumberOption((value) =>
        value.setName("value").setDescription("Valeur a ajouter")
      );
  }
  ...
  async run(context: CommandInteraction): Promise<void> {
    ...
    if (liste_duel.length != 0) {
      ...
      this.logger.debug(
        `${player.username} a gagné ${valeur_true} LP. Il possède ${Joueur_actuel.lp}`
      );
  ...
  /*async runOLD(context: CommandInteraction): Promise<void> {
    // TODO :: réussir à récupérer les LP actuels et les mettre à jour
    await context.deferReply();
    ...
    const playerId = player.id;
    //valeur
    const valeur = await context.options.get("value");
    const valeur_true = valeur.value;
    //on récupère le duel avec ce joueur
    const liste_duel = await this.duelMgr.getFor(playerId);
    //on vérifie que la liste n'est pas vide
    if (liste_duel.length != 0) {
      ...
      // on modifie les LP du joueur
      Joueur_actuel.lp = Joueur_actuel.lp + Number(valeur_true);
